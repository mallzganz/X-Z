...
require('./mallzganz.js')
nocache('./mallzganz.js', module => console.log(`${module} telah di update!`))
...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    fs.existsSync('./mallzganz.json') && zeroyt7.loadAuthInfo('./mallzganz.json')
    zeroyt7.on('connecting', () => {
        start('2', 'Menghubungkan...')
    })
    zeroyt7.on('open', () => {
        success('2', 'Terhubung Jangan Lupa Follow ig @m.a.l.l.zganz')
    })
    await zeroyt7.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./mallzganz.json', JSON.stringify(zeroyt7.base64EncodedAuthInfo(), null, '\t'))

    zeroyt7.on('chat-update', async (message) => {
        require('./mallzganz.js')(zeroyt7, message, _welkom)
    })
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        wel = `Halo @${anu_user} \nWelcome In ${mdata.subject} \nJangan Lupa PATUHI ATURAN GROUP`
