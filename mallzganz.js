...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
        ...
        const zero = fs.readFileSync ('./mallzganz/zerothumb.jpg')
...
"thumbnail": fs.readFileSync('./mallzganz/zero.jpg')
}
...
if (isGroupAdmins) return
var kic = `${sender.split("@")[0]}@s.whatsapp.net`
reply(` *「 LINK TERDETEKSI 」*\nKamu mengirimkan link chat, maaf kamu di kick dari grup :(`)
...
img = fs.readFileSync('./mallzganz/zero.jpg')
...
if (!isBotGroupAdmins) return reply("Bot Bukan Admin :)")
but = [
{ buttonId: '!antiwameon', buttonText: { displayText: 'On' }, type: 1 },
{ buttonId: '!antiwameoff', buttonText: { displayText: 'Off' }, type: 1 }
]
sendButton(from, "Silahkan pilih untuk anti WA.ME group", faketeks, but, mek)
break
case 'antiwameon' :
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply("Bot Bukan Admin :)")
if (isAntiwame) return reply('anti Wa.me sudah on')
_antiwame.push(from)
fs.writeFileSync('./database/antiwame.json', JSON.stringify(_antiwame))
reply(`\`\`\`✓Sukses mengaktifkan fitur anti WA.ME di group\`\`\` *${groupMetadata.subject}*`)
break
case 'antiwameoff' :
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply("Bot Bukan Admin :)")
if (!isAntiwame) return reply('anti Wa.me sudah off sebelumnya')
_antiwame.splice(from, 1)
fs.writeFileSync('./database/antiwame.json', JSON.stringify(_antiwame))
reply(`\`\`\`✓Sukses menonaktifkan fitur anti WA.ME di group\`\`\` *${groupMetadata.subject}*`)
break
case 'antilinkon' :
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins) return reply(mess.only.admin)
...
break
case 'stalkgithub':
if (args.length == 0) return reply(`Usernamenya Mana Tod\nContoh: ${prefix + command} mallzganz`)
username = args[0]
...
reply('TUHH NOMER OWNER KU, JIKA SUDAH MESAN SESUATU KIRIM BUKTI TF NYA🙏😇')
break
case 'bc':
...
break
case 'gopay':
qris = fs.readFileSync('./mallzganz/qrgopay.jpg')
but = [
{ buttonId: `${prefix}done`, buttonText: { displayText: 'UDAH TF' }, type: 1 }
]
sendButton(from, "GOPAY : SCAN QR YA WEY (Mikey)", faketeks, but, qris, mek)
break
case 'dana':
but = [
{ buttonId: `${prefix}done`, buttonText: { displayText: 'UDAH TF' }, type: 1 }
...
trans = `©Created : MALLZGANZ`
list = `✮Price List Diamond ML✮
MLBB FAST 

*_💎MOBILE LEGEND VIA ID💎_*
86💎💌18.000
172💎💌35.800
257💎💌53.700
344💎💌71.650
...
break
case 'ucpubg':
qris = fs.readFileSync('./mallzganz/pubg.jpg')
trans = `©Created : MALLZGANZ`
